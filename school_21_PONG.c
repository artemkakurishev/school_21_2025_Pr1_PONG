    ...
    for (int y = 0; y < HEIGHT_SCREEN; y++) {
        for (int x = 0; x < WIDTH_SCREEN; x++) {
            if (y == 0 || y == HEIGHT_SCREEN - 1) {
                ...
                if (x == mid / 2 - 1 - 4) {
                    printf("Player 1: %d", score_player_1);
                    x += score_width;
                } else if (x == mid + mid / 2 - 1 - 5) {
                    printf("Player 2: %d", score_player_2);
                    x += score_width;
...
void print_instructions() {
    printf("WELCOME in Pong!\n");
    printf("  Player 1: 'A' (up), 'Z' (down), Space (skip)\n");
    printf("  Player 2: 'K' (up), 'M' (down), Space (skip)\n");
    printf("First to 21 points wins!\n");
    printf("==============================\n\n");
}
 
// Artemiy added each turn to play
 
void print_turn(int current_player) {
    if (current_player == 1) {
        printf("Player 1's turn (A/Z/Space):\n");
    } else {
        printf("Player 2's turn (K/M/Space):\n");
    }
}
...
int main(void) {
    ...
    int score_player_1 = 0, score_player_2 = 0, current_player = 1;
    char inp = ' ', inp2 = ' ';
    int goal = -1;
    ...
    do {
        ...
        if (current_player == 1) {
            if (inp != 'a' && inp != 'A' && inp != 'z' && inp != 'Z' && inp != ' ') continue;
            if (inp == 'a' || inp == 'A') {
                if (rocket1_y > 2) {
                    rocket1_y -= 1;
                }
            ...
            if (inp != 'k' && inp != 'K' && inp != 'm' && inp != 'M' && inp != ' ') continue;
            if (inp == 'k' || inp == 'K') {
                if (rocket2_y > 2) {
                    rocket2_y -= 1;
                }
